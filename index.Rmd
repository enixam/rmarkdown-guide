...
欢迎来到《R Markdown 中文指南》的在线版。[R Markdown](https://rmarkdown.rstudio.com/) 是 [R](https://www.r-project.org/) 语言的一个扩展包(package)。它的主要目的是为数据分析工作提供一个交互式的环境, 用户可以即时地处理, 分析和可视化数据。工作完成后, 可以将一键把文档生成网页, PDF 和幻灯片等多种形式, 将文字, 代码和图片优美地组合呈现。

但是, R Markdown 的用途远不止此。本质上, 它只是为用户提供了一个组织内容的格式并负责将其导出, 而对具体的内容不加任何限制。因此, 无论你是否将 R 作为主要工具, 从事数据分析工作, 习惯编写代码, 或者专注于技术协作, R Markdown 都可以为你所用。 本书的 \@ref(rmarkdown-gallery) 节简要地展示了 R Markdown 丰富的用例, 你可以。相信你在读完本书之后, 也能在找到它的用武之地, 甚至加入 R Markdown 的开发者行列, 为它添加新的可能性。
